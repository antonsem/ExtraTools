...
namespace ExtraTools
{
	public class AudioPlayer : MonoBehaviour
	{
		...
		private void OnDisable()
		{
			...
			var sources = _thisObject.GetComponents<AudioSource>();
			...
			string musicMixerGroup = "Music", string sfxMixerGroup = "SFX")
		{
			...
			var sfxGroups = _mixer.FindMatchingGroups(sfxMixerGroup);

			if (sfxGroups.Length > 0)
			{
				_sfxMixer = sfxGroups[0];

				for (var i = 0; i < initializeSources; i++)
				{
					var source = _thisObject.AddComponent<AudioSource>();
			...
			var musicGroups = _mixer.FindMatchingGroups(musicMixerGroup);
		...
		{
			if (!volumeLabel.IsValid() || !_mixer.GetFloat(volumeLabel, out var currentVolume))
			{
				_musicSource.Stop();
		...
		public static void SetVolume(string volumeLabel, float volume, bool instant = false, Action callback = null)
		{
			...
			if (_activeCoroutines.TryGetValue(volumeLabel, out var coroutine))
		...
		{
			_mixer.GetFloat(volumeLabel, out var currentVolume);
